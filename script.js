...
// colours
const COLOR_BACKGROUND = "darkslategrey";
const COLOR_BALL = "white";
const COLOR_PADDLE = "navajowhite";
const COLOR_TEXT = "snow";
const COLOR_WALL = "navajowhite";

// text
const LABEL_FONT = "VT323, monospace";
const TEXT_FONT = "VT323, monospace";
...
const PupType = {
    EXTENSION: {color: "powderblue", symbol: "="},
    LIFE: {color: "palevioletred", symbol: "+"},
    STICKY: {color: "greenyellow", symbol: "~"},
    SUPER: {color: "darkgoldenrod", symbol: "s"}
}

// set up sound effects
var fxBrick = new Audio("brick.mp3");
var fxPaddle = new Audio("paddle.mp3");
var fxPowerup = new Audio("powerup.mp3");
var fxWall = new Audio("wall.mp3");
var fxout = new Audio("gameover.mp3");

// game variables
var ball, bricks = [], paddle, pups = [], particles = [];
...
function drawBall() {
    ...
    var radius = (ball.w + ball.h) / 2;
    ctx.beginPath();
    ctx.arc(ball.x - ball.w * 0.5 + radius / 2, ball.y - ball.h * 0.5, radius, 0, 2 * Math.PI, false);
    ctx.fill();
}
...
function drawPaddle() {
    ...
    var a = paddle.x - paddle.w * 0.5;
    var b = paddle.y - paddle.h * 0.5;
    var borderRadius = 10;
    ctx.beginPath();
    ctx.moveTo(a + borderRadius, b);
    ctx.lineTo(a + paddle.w - borderRadius, b);
    ctx.quadraticCurveTo(a + paddle.w, b, a + paddle.w, b + borderRadius);
    ctx.lineTo(a + paddle.w, b + paddle.h - borderRadius);
    ctx.quadraticCurveTo(a + paddle.w, b + paddle.h, a + paddle.w - borderRadius, b + paddle.h);
    ctx.lineTo(a + borderRadius, b + paddle.h);
    ctx.quadraticCurveTo(a, b + paddle.h, a, b + paddle.h - borderRadius);
    ctx.lineTo(a, b + borderRadius);
    ctx.quadraticCurveTo(a, b, a + borderRadius, b);
    ctx.fill();
    ctx.closePath();
}
...
function getBrickColor(rank, highestRank) {
    ...
    let h = "";
    if(fraction == 0) h = "#8B4513";
    else if(fraction <= 0.4) h = "#F4A460";
    else if(fraction <= 0.7) h = "#D2B48C";
    else h = "#FFEBCD";
...
function gameOverBox() {
    setTimeout(() => {                
        ...
        bigHighScoreEl.innerHTML = parseInt(localStorage.getItem(KEY_SCORE));
        if(score == scoreHigh){
...
//settings dimensions before game begins
setDimensions();
...
startGameBtn.addEventListener('click', () => {
    ...
    //restarting game
    if(startGameBtn.innerHTML == "Restart"){
        location.reload();
    }
});